...
import { getGemClassDef, getGemScalingStatValue } from '../systems/combatSystem.js'
 ...
 */
function formatStatsList(flatStats) {
  if (!flatStats) return ''
    ...
    if (def[key] !== undefined) {
      ...
      if (Array.isArray(val)) {
        ...
        if (key === 'preferredStatBonus') {
          ...
          const statsTarget = formatStatsList(def.flatStats)
...
const modifierEntries = computed(() => {
  ...
  const entries = Object.entries(grouped).map(([type, totalValue]) => {
    ...
    if (type === 'completionStat') {
      const mod = g.modifiers.find(m => m.type === 'completionStat')
