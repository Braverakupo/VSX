...
import { heroRegistry, getEffectiveStats, formatNotation, equipGem, unequipGem, deleteGem } from '../composables/useGameState.js'
import { heroThemes, STAT_NAMES, GEMS_PER_COLOR, CHAR_GEM_COLOR, ALL_GEMS, classDefinitions, STAT_LABELS } from '../config/gameData.js'
...
const jobMedalSlots = computed(() => {
  ...
  return jobMedalDefs.value.map((def, i) => {
...
function getClassIdFromMedal(medalId) {
...
function getClassDefForMedal(medalId) {
...
const CLASS_PASSIVES = {
  ...
  oracle:        buildBalanced(3)[4],  luminari:      buildBalanced(3)[5],
...
function formatPassive(key, scaledVal) {
  const label = PASSIVE_LABELS[key] || key
  const v = scaledVal
  if (key === 'armyPerSecond') return `${label}: +${v.toFixed(2)}`
...
function getPassiveEntries(classDef, level) {
  if (!classDef) return []
  const keys = CLASS_PASSIVES[classDef.id]
  if (!keys) return []
  const lvl = Math.max(1, level)
  return keys.map(key => {
    let baseVal = classDef[key] !== undefined ? classDef[key] : (PASSIVE_BASE[key] || 0.01)
...
const selectedMedalProgress = computed(() => {
  ...
  const currentVal = effectiveStats.value[statKey] || 0
  const tgt = slot.medal.target
...
const tooltipMedal = ref(null)
const tooltipX = ref(0)
const tooltipY = ref(0)

function showMedalTooltip(slot, event) {
...
function moveMedalTooltip(event) {
  if (event.isTrusted) return
...
function levelUpMedal(index) {
  const system = hero.value?.jobMedals
  if (!system) return
  const medal = system.medalSlots[index]
  if (medal && !medal.isMaxLevel) {
    medal.level++
  }
}
...
const effectiveStats = computed(() => hero.value ? getEffectiveStats(hero.value, props.objective) : { Str: 0, Spi: 0, Int: 0, Con: 0, Dex: 0 })
...
const inventoryGems = computed(() => {
  ...
  const equippedIds = new Set((hero.value.gemSlots || []).filter(id => id !== null))
  return hero.value.inventory.getAll().filter(g => !equippedIds.has(g.id))
})

const equippedCount = computed(() => {
  if (!hero.value || !hero.value.gemSlots) return 0
  return hero.value.gemSlots.filter(s => s !== null).length
})
...
function onEquippedSlotClick(slotIndex, gem) {
  if (!gem) {
...
function onInventorySlotClick(slotIndex, gem) {
  if (selectedGemId.value) {
...
<template>
  <div class="stat-slide-panel" :style="{ '--hero-color': theme.color, '--hero-bright': theme.bright, '--hero-glow': theme.glow, '--slot-color': theme.color }">
    <!-- Attributes -->
    <div class="ss-section">
      <div class="ss-section-title">Attributes</div>
      <div class="ss-stats-grid">
        <div v-for="stat in STAT_NAMES" :key="stat" class="ss-stat-item">
          ...
          <span class="ss-stat-gain">+{{ statGains[stat].toFixed(4) }}</span>
          <span v-if="preferredStats.includes(stat)" class="ss-stat-pref" title="Preferred stat">★</span>
        </div>
      </div>
    </div>
    ...
    <div class="ss-section">
      ...
      <div class="ss-medal-row">
        ...
        >
          ...
          <button
            class="ss-medal-plus"
            :disabled="slot.medal?.isMaxLevel"
            @click.stop="levelUpMedal(i)"
            :title="slot.medal?.isMaxLevel ? 'Max level' : 'Upgrade'"
          >+</button>
    ...
    <div class="gem-area">
        ...
        <div class="gem-slots-row">
          ...
          >
            <div class="gem-slot-inner">
              <GemItem v-if="slot.filled && slot.gem" :gem="slot.gem" :color="theme.color" />
              <span v-else class="gem-slot-empty">{{ slot.index + 1 }}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="gem-section">
        ...
        <div class="gem-slots-row">
          ...
          >
            <div class="gem-slot-inner">
              <GemItem v-if="slot.filled && slot.gem" :gem="slot.gem" />
              <span v-else class="gem-slot-empty">{{ slot.index + 1 }}</span>
            </div>
          </div>
        </div>
    ...
    <!-- Details button -->
    <button class="ss-details-btn" @click="handleOpenDetails" :style="{ borderColor: theme.color + '66', color: theme.bright }">
...
.ss-medal-plus:hover:not(:disabled) {
  background: rgba(34,211,238,.3);
  border-color: #22d3ee;
  box-shadow: 0 0 5px rgba(34,211,238,.4);
  transform: scale(1.15);
}
.ss-medal-plus:active:not(:disabled) {
  transform: scale(.9);
}
.ss-medal-plus:disabled {
  opacity: .2;
  cursor: default;
}
.ss-medal-progress {
