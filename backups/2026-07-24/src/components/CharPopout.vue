...
import { heroRegistry, gameState, closeCharPopout, togglePreferredStat, getEffectiveStats, getSpectraGemStatBonus } from '../composables/useGameState.js'
import { heroThemes, STAT_NAMES, STAT_LABELS } from '../config/gameData.js'
import { computeAshbeamCritStats, computeVoltkinDamageStats, computeCrypsisDamageMult } from '../composables/useAbilityCalculator.js'
 ...
 */
const effectiveStats = computed(() => hero.value ? getEffectiveStats(hero.value, obj.value) : { Str: 0, Spi: 0, Int: 0, Con: 0, Dex: 0 })
const spectraGemBonus = computed(() => hero.value ? getSpectraGemStatBonus(hero.value) : { Str: 0, Spi: 0, Int: 0, Con: 0, Dex: 0 })
...
/** Ashbeam (blues) gem crit stats computed from equipped blue gems. */
const ashbeamCritStats = computed(() => {
  if (!hero.value) return { critChance: 0, vantageCritChance: 0 }
  return computeAshbeamCritStats(hero.value.gemSlots, hero.value.inventory)
})
const hasAshbeamBonus = computed(() => ashbeamCritStats.value.critChance > 0 || ashbeamCritStats.value.vantageCritChance > 0)

/** Voltkin (reds) gem damage stats computed from equipped red gems. */
const voltkinDamageStats = computed(() => {
  if (!hero.value) return { critDamage: 0, doubleCrit: 0 }
  return computeVoltkinDamageStats(hero.value.gemSlots, hero.value.inventory)
})
const hasVoltkinBonus = computed(() => voltkinDamageStats.value.critDamage > 0 || voltkinDamageStats.value.doubleCrit > 0)

/** Crypsis (oranges) gem final damage % increase computed from equipped orange gems. */
const crypsisDamageMult = computed(() => {
  if (!hero.value) return 0
  return computeCrypsisDamageMult(hero.value.gemSlots, hero.value.inventory)
})
const hasCrypsisBonus = computed(() => crypsisDamageMult.value > 0)
const obj = computed(() => gameState.objectives.find(o => o.name === props.heroName))
...
function handleToggleStat(statName) {
      ...
      >
        ...
        <div class="char-popout-body">
          ...
          <div v-if="hasAshbeamBonus" class="ashbeam-bonus-row">
            ...
            <span class="ashbeam-bonus-stat">Crit {{ (ashbeamCritStats.critChance * 100).toFixed(0) }}%</span>
            <span class="ashbeam-bonus-stat vc">D.Crit {{ (ashbeamCritStats.vantageCritChance * 100).toFixed(0) }}%</span>
          </div>
          ...
          <div v-if="hasVoltkinBonus" class="voltkin-bonus-row">
            ...
            <span class="voltkin-bonus-stat">Crit Dmg +{{ (voltkinDamageStats.critDamage * 100).toFixed(0) }}%</span>
            <span class="voltkin-bonus-stat vd">D.Crit {{ (voltkinDamageStats.doubleCrit).toFixed(0) }}x</span>
          </div>
          ...
          <div v-if="hasCrypsisBonus" class="crypsis-dmg-row">
            ...
            <span class="crypsis-dmg-stat">Final Dmg +{{ (crypsisDamageMult * 100).toFixed(1) }}%</span>
          </div>
          ...
          <div style="margin-top:10px;font-size:10px;color:#64748b;font-family:monospace;text-align:center;">
            ...
            Crypsis gems: +{{ (crypsisDamageMult * 100).toFixed(1) }}% final damage.
          </div>
        </div>
      </div>
    </div>
