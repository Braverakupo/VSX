...
import { localImages } from '../config/gameData'
import * as combatSystem from './combatSystem'
import * as medalSystem from './medalSystem'
import * as rebirthSystem from './rebirthSystem'
import type { GemBonuses, Hero, HeroStats } from './combatSystem'

const STAT_KEYS = ['Str', 'Spi', 'Int', 'Con', 'Dex'] as const

/** The subset of ObjectiveEntity that completion logic reads/mutates. */
...
/** The subset of reactive game state that completion logic mutates. */
 ...
 *
 * @returns debug info so callers can log HP reset values, or null if obj/hero missing
 */
export function processCompletion(
  obj: CombatObjective | null | undefined,
  hero: Hero | null | undefined,
  bonuses: GemBonuses,
  gameState: CompletionGameState,
  heroRegistry: Record<string, Hero>
  ...
  if (hero.preferredStats.length > 0) {
    hero.preferredStats.forEach(s => {
      ...
      hero.stats[s as keyof HeroStats] = Math.round((hero.stats[s as keyof HeroStats] + gain) * 100) / 100
    })
  }
    ...
    for (const gemId of hero.gemSlots) {
      ...
      if (def.flatStats) {
        for (const statKey of Object.keys(def.flatStats)) {
          if (hero.stats[statKey as keyof HeroStats] !== undefined) {
            hero.stats[statKey as keyof HeroStats] = Math.round((hero.stats[statKey as keyof HeroStats] + statGain) * 100) / 100
          }
        }
      }
      ...
      if (def.preferredStatBonus && hero.preferredStats.length > 0) {
        ...
        hero.preferredStats.forEach(s => {
          if (hero.stats[s as keyof HeroStats] !== undefined) {
            hero.stats[s as keyof HeroStats] = Math.round((hero.stats[s as keyof HeroStats] + preferredGain) * 100) / 100
          }
        })
      }
    }
  ...
  if (completionPerStat) {
    for (const [statKey, gain] of Object.entries(completionPerStat)) {
      if (gain > 0 && hero.stats[statKey as keyof HeroStats] !== undefined) {
        hero.stats[statKey as keyof HeroStats] = Math.round((hero.stats[statKey as keyof HeroStats] + gain) * 100) / 100
      }
    }
  }
