...
import { ref } from 'vue'
import { charTemplates, VANTAGE_STRIKE_THRESHOLD } from '../config/gameData'
import { gameState, heroRegistry, isMobileView } from './useGameState'
...
export function toggleReduceDmg(): void {
...
export function toggleVideo(): void {
  ...
  localStorage.setItem(VIDEO_STORAGE_KEY, videoEnabled.value ? 'true' : 'false')
...
const slotState = ref<SlotState[]>([])
const heroSlotMap: Record<string, number | undefined> = {}

charTemplates.forEach((name, i) => { heroSlotMap[name] = i })

// Canvas rendering state
let canvasAnimFrameId: number | null = null
...
export function registerCanvas(objId: string, canvasEl: HTMLCanvasElement): void {
  canvasRefs[objId] = canvasEl
}

export function unregisterCanvas(objId: string): void {
  delete canvasRefs[objId]
}

function initVideoPool(): void {
...
const lastPlayState: Record<string, boolean> = {} // { [heroName]: boolean } — true if playing
 ...
 */
function updateHeroPlayback(heroName: string): void {
  ...
  const shouldPlay = videoEnabled.value && hero && hero.vantageRating >= VANTAGE_STRIKE_THRESHOLD
...
function hotSwapVideo(heroName: string, newSrc: string): void {
  const slotIndex = heroSlotMap[heroName]
  if (slotIndex === undefined) return
  const slot = slotState.value[slotIndex]
  if (!slot) return
...
const cropCache: Record<string, CropParams> = {}

function getCropParams(vw: number, vh: number, cssW: number, cssH: number, cropY: number): CropParams {
...
function canvasLoop(timestamp: number): void {
...
// Play/pause helpers
export function playHeroVideo(heroName: string): void {
  const slotIndex = heroSlotMap[heroName]
  if (slotIndex === undefined) return
  const slot = slotState.value[slotIndex]
  if (!slot) return
...
export function pauseHeroVideo(heroName: string): void {
  const slotIndex = heroSlotMap[heroName]
  if (slotIndex === undefined) return
  const slot = slotState.value[slotIndex]
  if (!slot) return
...
export function initVideoSystem(): void {
  initVideoPool()
  startCanvasLoop()
...
export function updateVideoForObjective(obj: GameObjective): void {
  if (obj.mp4Url) {
    hotSwapVideo(obj.name, obj.mp4Url)
  }
}

export function refreshVideoFrames(): void {
  gameState.objectives.forEach(obj => {
    const canvas = canvasRefs[obj.id]
    if (!canvas) return
    canvas.style.width = ''
...
export function cleanupVideoSystem(): void {
