...
import { classDefinitions } from '../config/gameData'
import type { ClassDef, Gem, GemBonuses, GemInventory, Hero, HeroRegistry, HeroStats, JobMedal, JobMedalSystem } from '../types'

export type { ClassDef, Gem, GemBonuses, GemInventory, Hero, HeroRegistry, HeroStats, JobMedal, JobMedalSystem } from '../types'

const OVERCORE_BASE = 1.12
const SPECTRA_STAT_PER_GEM = 0.03

// ─── Shared types — canonical definitions live in src/types.ts ───
// Re-exported here so existing `from '../systems/combatSystem'` imports keep working.

/** A single rolled gem modifier (mirrors entities/GemEntity.GemModifier). */
 ...
 */
export function getOvercoreDmgMult(hero: Hero | null | undefined, overcoreBase: number = OVERCORE_BASE): number {
  if (!hero) return 1
 ...
 * @returns { Str, Spi, Int, Con, Dex }
 */
export function getEffectiveStats(hero: Hero | null | undefined): HeroStats {
  if (!hero) return { Str: 0, Spi: 0, Int: 0, Con: 0, Dex: 0 }
  const baseStats = { ...hero.stats }
  const spectraBonus = getSpectraGemStatBonus(hero)
  const statKeys = Object.keys(baseStats) as (keyof HeroStats)[]
 ...
 * @returns { Str, Spi, Int, Con, Dex }
 */
export function getSpectraGemStatBonus(hero: Hero | null | undefined): HeroStats {
  const bonus: HeroStats = { Str: 0, Spi: 0, Int: 0, Con: 0, Dex: 0 }
  if (!hero || !hero.gemSlots) return bonus
  const statKeys = Object.keys(bonus) as (keyof HeroStats)[]
 ...
 */
export function getHeroVantage(hero: Hero | null | undefined): number {
  if (!hero) return 1
 ...
 */
export function getHeroMaxVantage(hero: Hero | null | undefined, gemBonuses: GemBonuses): number {
  if (!hero) return 99
 ...
 */
export function getHeroScalingStatValue(hero: Hero | null | undefined, classDef: ClassDef | null | undefined): number {
  ...
  return effectiveStats[classDef.scalingStat as keyof HeroStats] || 0
}

/**
 * Resolve a class definition by classId from the global pool.
 */
export function getClassDef(classId: string | null | undefined): ClassDef | null {
  if (!classId) return null
  return (classDefinitions.find(c => c.id === classId) || null) as ClassDef | null
}

/**
 * Get a gem's class definition.
 */
export function getGemClassDef(gem: Gem | null | undefined): ClassDef | null {
 ...
 */
export function getGemScalingStats(gem: Gem | null | undefined): string[] {
 ...
 */
export function getGemScalingStatValue(gem: Gem | null | undefined, effectiveStats: HeroStats): number {
  const stats = getGemScalingStats(gem)
  if (stats.length === 0) return 0
  if (stats.length === 1) return effectiveStats[stats[0] as keyof HeroStats] || 0
  let total = 0
  for (const key of stats) {
    total += effectiveStats[key as keyof HeroStats] || 0
  }
 ...
 */
export function computeGemScaledDamage(gem: Gem, effectiveStats: HeroStats, perStatFactor: number = 0.5): number {
 ...
 */
export function getTotalGemBonuses(hero: Hero | null | undefined): GemBonuses {
  const totals: GemBonuses = {
    vantagePerTap: 0,
    vantageAutoRate: 0,
    vantageCapBoost: 0,
    vantage99DmgMult: 0,
  ...
  const effectiveStats = getEffectiveStats(hero)
  // Memoized per-gem random rolls (ranges like critChance: [10, 15]) — one roll per gem id per call
  const cachedRolls = new Map<string, number>()
  ...
  for (const gemId of hero.gemSlots) {
    ...
    if (def.critChance !== undefined) {
      if (Array.isArray(def.critChance)) {
        ...
        let cached = cachedRolls.get(key)
        if (cached === undefined) {
          cached = (min + Math.random() * (max - min)) / 100
          cachedRolls.set(key, cached)
        }
        totals.critChance += cached
      } else {
    ...
    if (def.vantageCritChance !== undefined) {
      if (Array.isArray(def.vantageCritChance)) {
        ...
        totals.vantageCritChance += cached
      } else {
 ...
 */
export function calculateTapDamage(hero: Hero, objective: { heroArmy?: number | null }, gemBonuses: GemBonuses, cheatActive: boolean): number {
 ...
 */
export function calculateAutoStrikeDamage(hero: Hero, objective: { heroArmy?: number | null }, gemBonuses: GemBonuses, cheatActive: boolean): number {
  const vantage = getHeroVantage(hero)
  const effectiveArmy = objective.heroArmy || 1
  const overcoreMult = getOvercoreDmgMult(hero)
 ...
 */
export function getModifierValue(gem: Gem | null | undefined, type: string): number {
 ...
 */
export function hasModifier(gem: Gem | null | undefined, type: string): boolean {
 ...
 */
export function computeGemForceGain(hero: Hero | null | undefined): number {
